        ...
        <div class="w-full lg:w-5/12 px-4">
            <div class="relative w-full mb-3">
                ...
                <div class="flex justify-content-center">
                    <Dropdown
                        v-model="student_data.grade_level"
                        :options="grades"
                        placeholder="Select Grade"
                        class="w-full md:w-14rem"
                        @change="selectClass(student_data.grade_level)"
                    />
                </div>
                <p v-if="errors?.grade_level" class="text-xs mt-1 text-red-500">
                    {{ errors?.grade_level[0] }}
                </p>
            </div>
        </div>
        <div v-if="gradeClasses.length > 0" class="w-full lg:w-5/12 px-4">
            <div class="relative w-full mb-3">
                <label
                    class="block uppercase text-blueGray-600 text-xs font-bold mb-2"
                >
                ...
                <div class="flex justify-content-center">
                    <Dropdown
                        v-model="student_data.class"
                        :options="gradeClasses"
                        optionLabel="class"
                        placeholder="Select Class"
                        class="w-full md:w-14rem"
                    />
                    <p v-if="errors?.class" class="text-xs mt-1 text-red-500">
                        {{ errors?.class[0] }}
                    </p>
                </div>
            </div>
        </div>
        <div class="flex lg:w-2/12 mx-4 lg:m-0 justify-center">
            <button
                v-if="!student_data.class"
                class="rounded py-2 px-6 cursor-not-allowed bg-slate-500 text-white"
            >
                <span> Search </span>
            </button>
            <button
                v-else
                @click="index(student_data)"
...
const { takeAttendance, loading, isClass, index, students, classDailyReport } =
    useAttendance();
